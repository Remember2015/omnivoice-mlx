...
- RTF 是合成耗时除以生成音频的时长，越小越快。0.1 就是一秒钟的话花 0.1 秒算出来。
- 短句指 7、17、34 字三句合起来算；长段是一段读出来 46 秒的文字。
- 步是去掩码的迭代次数。这个模型不一个字一个字往外蹦，整句一起反复去掩码，步数越少越快。官方默认 32 步；
  这里的 16 步还另外省了两处重复计算（前缀 KV 缓存、无条件分支隔步复用），都在研究记录里。
- 出声延迟从调用算到整句音频出来。架构上没有流式，没有先出一点边放边算这回事，第一个包就是整句。
...
参考音自己准备，仓库里不带：一段录下来的嗓子属于说话的那个人。3–4 秒干净单声道就够，转写必须是它真说的那句，
因为转写进 prompt，不是标签。跑 `bench/` 时用环境变量指过去：
...
- 激活换 fp16。M2 的 GPU 没有原生 bf16，MLX 靠转换模拟，所以 bf16 和 fp32 一样慢；换 fp16 快 12–15 %。
- 步数减半。官方 32 步，砍到 16 步三个质量指标都不动；要到 8 步自然度才掉 5 %。
- 缓存 prompt 的 K/V。风格 + 文本 + 参考音那段每 8 步才重算一次，中间的步只把目标 token 送进去。
- 无条件分支隔步复用。分类器自由引导的那条分支每 3 步算一次，中间沿用上次的结果，省 20 %，指标不动。

8-bit 量化本身不提速。每步是 M ≈ 200–400 的 GEMM，卡在 kernel 发射而不是带宽，量化矩阵乘在这个尺寸下反而更慢。
它省的是内存：常驻 1.99 GB 降到 1.56 GB。

试了没用的都留在日志里：自定义 Metal GEMM 内核（simdgroup 版只在 M ≤ 80 且 N ≥ 4096 赢 5–30 %，其余慢 10–15 %）、
CFG 截断（声纹 −0.05～−0.08）、置信度阈值自适应步数、4 步（出整句错）、分句续接（自然度 2.95 → 2.65）、
`mx.compile`。
...
**权重不归这个许可管。** k2-fsa 的代码是 Apache-2.0，预训练权重是 CC-BY-NC，因为训练数据有约束。
`scripts/convert.py` 转出来的、以及上面那份 HF 权重，都是它的衍生物，同样非商用、要署名。
