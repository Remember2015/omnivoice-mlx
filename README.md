...
- **fp16 激活**。M2 的 GPU 没有原生 bf16，MLX 靠转换模拟，所以 bf16 和 fp32 一样慢，换 fp16 就快 12–15 %。
- **16 步**。官方 32 步，砍一半三个质量指标都不动；要到 8 步自然度才掉 5 %。
- **前缀 KV 缓存**。prompt（风格 + 文本 + 参考音）的 K/V 每 8 步才重算一次，中间的步只把目标 token 送进去。
- **无条件分支隔步复用**。分类器自由引导的那条无条件分支每 3 步算一次，中间沿用上次的结果：−20 %，指标不动。

**8-bit 量化本身不提速**：每步是 M ≈ 200–400 的 GEMM，卡在 kernel 发射而不是带宽，量化矩阵乘在这个尺寸下反而更慢。
它省的是内存和包体（1.99 → 1.56 GB 常驻）。
