...
- 计时：一个进程加载一次模型，变体按句轮转交错，3 轮中位数，先各预热一次。
  RTF =（unmask + codec 解码）/ 原始时长（T × 40 ms），不含后处理。全部走 `bench/benchlock.sh`（排他锁 + 等空载）。
  绝对值随负载漂 10–40 %，只比同进程交错的数。
- 质量：20 句中文集，Fun-ASR-Nano 回读算 CER，CAM++ 算对参考音的相似度，UTMOS22-strong 算自然度。三个模型都不在
  本仓库，绝对值不能跨套比。参考音 3.7 s（92 token），2026-09-16 用 Demucs 去过背景音乐，前后的质量数不能混比。
- 变体名 `s<步数>[-kv<n>][-ue<n>][-cfg<比例>]`。`uncond_every` 默认值后来从 1 改成 3，早期表里的 `s8-kv4` 今天要写
  `s8-kv4-ue1`。
