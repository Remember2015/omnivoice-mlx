...
本项目，int8 g64 权重 + fp16 激活，列名是 `SamplerConfig` 的参数（`cache_refresh=0` 表示不用 KV cache）：
...
| int8 g64 | bf16 | 1.56 GB | 829 MB | 37.1 / 42.3 / 64.4 | 0.514 |
| **int8 g64** | **fp16** | 1.56 GB | 829 MB | **27.8 / 31.5 / 47.1** | **0.381** |

- M2 没有原生 bf16，bf16 与 fp32 同速，fp16 比它们快 12–15 %。
- 量化更慢只对 bf16 激活成立，换 fp16 激活后与 fp16 GEMM 持平或略快；默认取 int8 + fp16。
- 每步约 15 ms 固定开销：一行从 198 token 加到 254 只多 7 %，254 → 391 才多 44 %。
- 八个 codebook 的输出头不量化。改 fp16 不提速，UTMOS 2.835 → 2.794，所以 `--head-dtype` 默认 float32。
...
四个 int8 GEMM 占一个 step 的 67 %，T=126 时 77 %。GPU 占用率 98–99 %；GEMM 速率 5.9 TFLOPS，是峰值 13.6 的 43 %，
长文本时 7.1，占 52 %。
...
| 本项目 MLX | int8 + fp16 | 0.225 | — | 15.4 / 16.9 / 29.6 |
