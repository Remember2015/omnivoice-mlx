...
装和用看 [../README.md](../README.md)。这里是过程：每节写明怎么量的，赢的输的都留着。

M2 Max 12 核 / 38 核 GPU / 32 GB，macOS 26.6，MLX 0.32.2，2026-09-09 起。
...
## 3. 采样：KV cache 有效，CFG 截断掉音色，自适应步数无效
...
## 6. 吞吐：多句 batch B=4–8 拿 −25 %，顺带踩到 MLX 的缓存坑
...
**B≥4 就到平台**，再大只多吃内存。交互式没用（首批延迟按 B 倍增），这是段落和离线合成的选项。

坑：第一版测出来「越 batch 越慢」（B=1 0.107 → B=8 0.157，GPU 忙闲 97 % → 65 %）。主机搭图只 11–74 ms，不是它。
追下去是 **MLX 的缓冲区缓存默认无上限**：一次大 batch 之后缓存里留着几 GB 奇形尺寸的空闲 buffer，之后每次分配
...
打进一个 unmask 循环 0.053——**chunk 并行只再省 6–8 %**，每步已经 700+ token，GPU 早就吃满。一段 46 s 的文字 2.5 s
...
**自定义 Metal GEMM：做了两代，输了。** 第一代多行 GEMV 式（`kernels.py`）数值正确但慢 1.3–5 倍——标量 FMA 每 4 个
...
两代算子证明的是：**MLX 的量化 GEMM 在这个模型的形状上已经贴着硬件**，它唯一的结构性浪费是把 M 补到 32
（M=32 → 33 时 56 → 90 µs），8 行 tile 拿到的就是这一块；它的 mma 已经跑满机器峰值的 55–75 %。
单句延迟上算子层没有余地了，剩下的收益全在算法层（第 4、5 节）和多句 batch。

## 9. codec 瘦身：常驻 1.57 → 0.87 GB，顺带去掉 transformers
...
## 10. text normalization：CER 7.09 % → 0.33 %，贴到底噪
...
原文直接喂会丢意思：`-5 度` 念「五度」、`3.5%` 丢「百分之」、`14:30` 念「十四三零」、`1/3` 念「一四三」。
两个现成底座各有洞，谁也不比谁完善：zh_normalization 把 `3500 台` 念成「三五零零台」、`¥199` 里的 ¥ 念成「U」；
wetext 把带空格的 `2024 年` 按基数读；两边都把 `GPT-4` 念成「GPT 负四」。加起来只有 5 种形状，补上就贴到底噪。
最终用 wetext 做底座 + 前置修补（`omnivoice_mlx/ttstext.py`），默认关，因为它另需两个包。
...
差在「少做事」，不在算子：mlx-audio 每步跑两次前向（cond、uncond 各一次 batch-1），8 个 head 对整行算 logits 再切
...
## 12. 官方 torch 走 MPS：能跑，本移植仍快 5 倍（2026-09-18）
