...
- CER 和 speaker similarity 区分不出 8 步和 32 步（0.735–0.746 全在噪声内），UTMOS 能：16 → 8 步稳定掉 0.13，
  8 → 6 → 4 再各掉 0.1、0.26。
- KV cache 在 16 步上不掉 UTMOS，6 步起开始有损失（s6 → s6-kv6 −0.06）。
...
**MLX 的缓冲区缓存默认无上限**：大 batch 之后滞留几 GB 空闲 buffer，之后每次分配都走慢路径，连 B=1 都从
0.107 退到 0.162。`mx.set_cache_limit(512 MB)` 已写进 `OmniVoiceTTS.__init__`，不设就量不出上面这张表。
