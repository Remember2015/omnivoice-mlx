...
**一句话结论**（M2 Max，中文单音色克隆，Fun-ASR 回读 CER + CAM++ 相似度做质量门）。对照组：

| 实现 | 框架 | 设备 | 精度 | 步数 | RTF 短句 | 整句延迟 | |
|---|---|---|---|---:|---:|---|---|
| 官方 | torch | CPU | fp32 | 32 | ≈2–3 | — | 基准 |
| 官方 | torch | CPU | fp32 | 8 | 0.8–1.7 | — | |
| 官方 | torch | MPS | fp32 | 32 | 0.98 | — | 第 19 节 |
| 官方 | torch | MPS | fp16 | 32 | 1.14 | — | 第 19 节 |
| 官方 | torch | MPS | fp32 / fp16 | 8 | 0.36 / 0.39 | — | 第 19 节 |
| mlx-audio | MLX | GPU | bf16 | 32 | 0.68 | 1.8–3.4 s | 上一轮的数 |

本项目（kv = 每几步刷新一次前缀 KV，ue = 无条件分支每几步算一次）：

| 精度 | 步数 | kv | ue | RTF 短句 | RTF 长段 | 整句延迟 | CER | sim | UTMOS | |
|---|---:|---:|---:|---:|---:|---|---:|---:|---:|---|
| fp16 | 32 | — | 1 | 0.41 | 0.25 | 1.0–1.6 s | 0.5 % | 0.740 | 2.83 | |
| **8bit+fp16** | **16** | **8** | **3** | **0.106** | **≈0.08** | **0.23–0.44 s** | **0.4 %** | **0.747** | **2.84** | **默认** |
| 8bit+fp16 | 16 | 8 | 1 | 0.133 | 0.10 | 0.27–0.54 s | 0.7 % | 0.737 | 2.84 | 第一版 |
| 8bit+fp16 | 16 | 8 | 3 | 0.080 | — | — | — | — | — | batch B=4–8 |
| 8bit+fp16 | 8 | 4 | 1 | 0.075 | 0.057 | 0.16–0.32 s | 0.4 % | 0.738 | 2.69 | |
| fp16 | 6 | 6 | 1 | 0.06 | 0.044 | 0.13–0.22 s | — | — | 2.53 | |

假流式（按分句提前一句合成，第 10 节）：首音频 165 ms（8 步）/ 286 ms（16 步），之后零断流。
去掉的：CFG 截断（相似度 −0.05～−0.08）、置信度阈值自适应步数（不触发反而慢）、4 步（speaker similarity −0.03、UTMOS −0.5、整句出错）、
...
- **量化更慢**：这里每步是 M ≈ 200–400 行的 GEMM，卡的是算力和 kernel 启动开销，不是带宽，`quantized_matmul` 在这个 M 下比
  fp16 GEMM 慢 15–30 %。量化的价值只在内存（1.99 → 1.36 GB）和包体（1242 → 609 MB）。
  和 CuteTTS（自回归、M = 1–2，8-bit backbone −23 %）正好相反。
- 每步有约 15 ms 固定开销（198 → 254 token 只多 7 %，254 → 391 多 44 %）：28 层 × ~26 个 kernel 的启动和调度，
...
以及一处「粤菜」→「月菜」和一处「查一下」→「查下」（各 4.5 %）；没有一句 > 20 %，没有超长生成 / 复读。
...
- **CFG 截断**（`cfg_until`，后半步不跑 uncond 分支）：省 12–18 %，但相似度掉 0.05–0.08（0.75 → 0.69 / 0.67），最低值掉到
...
`--bits 4 --quantize-embed --quantize-heads` 连 151k 词表 embedding 和 8 个 head 一起 4-bit），四种变体装进同一个进程按句交错
（`bench/bench_models.py`，codec 共享），3 轮中位数：

| 变体 | 权重 | 32 步 ms/step（短/中/长） | RTF 32 步 | RTF 8 步 |
|---|---:|---|---:|---:|
...
在这批中文短句上没有出现，8 → 6 才是这台机器上速度 / 质量的拐点。单句 RTF 的下限约 0.06（s6-kv6），质量不掉；
再往下只能靠掉质量换。
...
fp32 parity：与官方仍逐 token 一致，快慢路径同种子 100 % / 100 % / 99.9 %。速度：16.9 vs 17.3 ms/step，噪声内——
沿 N 拼 GEMM 不缩短 K 循环延迟，少掉的小 kernel 每层 ~0.04 ms 淹没在噪声里。

**算子层剩下的唯一办法**是自定义 Metal 的 split-K 量化 GEMM（把 K 循环切成几段并行再归约），估计短句每步 −30 %，
长句 −10 %，工作量是天级的，没做。`mx.compile` 只融合 elementwise 链，对这个结构 ≤ 5 %，且按段切片的整数会固化进图里，也没做。
...
MLX 唯一的结构性浪费是把 M 补到 32（M=32 → 33 时耗时 56 → 90 µs），8 行 tile 拿到的就是这一块；它的 mma 已经跑满
机器峰值的 55–75 %；反量化 + 搬运阶段和 mma 不重叠（M=252 时 276 + 142 ≈ 394 µs），tile 做厚就掉占用率；
N=1024 的形状只有 N/BN 个 threadgroup，并行度不够，split-K 又要多启动一次 kernel（10–14 µs）吃掉差价。
...
只有 7 字句快 6 %（它的缓存步是 39 / 78 行），17 字以上一行都碰不到赢的区间；确定性 parity 7 / 34 字 100 %、17 字 94 %
...
**GPU 一直是满的，没有调度气泡可填**（`async_eval` 预排已经把主线程藏干净了）。效率不到一半的原因，按依赖链微基准
...
### 16. 低成本的优化做完了；多句 batch：修掉 MLX 缓存坑后 B=4–8 拿 −25 %
...
- **`mx.compile`**：每步 ~770 个 kernel 里可融合的只有 silu×up、残差加、Gumbel 那几个 elementwise 链，估计 ≤ 5 %，而按段切片的
  Python 整数会被固化进图里要按 (P, T) 重 trace，没做。固定开销的真解是更少的 kernel（自定义 Metal 融合层），工作量大。
